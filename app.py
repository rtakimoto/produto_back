...
def update_produto(form: ProdutoUpdateSchema):
    ...
    logger.debug(f"Atualizando produto de nome: '{produto.nome}'")
    # criando conexão com a base
    session = Session()
    # fazendo a remoção
    count = session.query(Produto).filter(Produto.nome == produto.nome).update({'quantidade': produto.quantidade, 'valor': produto.valor})
    session.commit()
    # fazendo a busca
    produto = session.query(Produto).filter(Produto.nome == produto.nome).first()

    if count:
        # retorna a representação da mensagem de confirmação
        logger.debug(f"Atualizado produto #{produto.nome}")
        return apresenta_produto(produto), 200
    else:
        # se o produto não foi encontrado
        error_msg = "Produto não encontrado na base :/"
